    ...
    [@b.grid items=clazzes var="clazz"]
      [@b.row]
        ...
        [@b.col width="17%" title="期末总评成绩"]
          ...
          [#assign openEnd=false][#list gradeInputSwitch.types as t][#if t.id==2][#assign openEnd=true][#break/][/#if][/#list]
          ...
          [#if (gradeStates.get(clazz).getState(EndGa).confirmed)!false]
            ...
            [#if gradeInputSwitch.checkOpen() && openEnd && !(gradeStates.get(clazz).getState(EndGa).published)!false]
            [@b.a  href="!revokeGa?clazzId="+clazz.id onclick="return bg.Go(this,null,'确定撤回?')"]撤回[/@]
            [/#if]
          [#else]
            [#if gradeInputSwitch.checkOpen() && openEnd][@b.a href="!clazz?clazzId=${clazz.id}" target="_blank"]录入[/@][/#if]
          [/#if]
        [/@]
