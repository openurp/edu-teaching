...
/**
 * 判断是否相同的活动 same activity [subject,users,places,vaildWeeks]
 */
function isSameActivity(other) {
  return this.canMergeWith(other) && (this.vaildWeeks == other.vaildWeeks);
}
/**
 * 合并相同的教学活动 same [subject,users,places,remark] can merge
 */
function canMergeWith(other) {
  return (this.users == other.users && this.subject == other.subject && this.places == other.places);
}
...
// 缩略连续周
function marshalContinue(result, weekOccupy, from, start) {
  var cycle = 1;
  var i = start + 2;
  for (; i < weekOccupy.length; i += 2) {
    if (weekOccupy.charAt(i) == '1') {
...
function marshal(weekOccupy) {
  ...
  while (i < weekOccupy.length) {
    ...
    if (post == '1') {
      start = marshalContinue(result, weekOccupy, from, start);
    }
...
/**
 * marshal style is or --------------------------- -------------------- | odd3-18
 * even19-24,place | | odd3-18 | -------------------------- --------------------
 */
function marshalValidWeeks() {
  if (this.places !== "") {
    return marshal(this.vaildWeeks) + "," + this.places;
  } else {
  ...
  for (var i = 0; i < this.unitCounts; i++) {
    if (this.activities[i].length > 1) {
      for (var j = 1; j < this.activities[i].length; j++) {
        this.activities[i][0].vaildWeeks = bitor(
...
// merger activity in every unit by activity subject.
function mergeBySubject() {
  for (var i = 0; i < this.unitCounts; i++) {
    if (this.activities[i].length > 1) {
      // O(n^2)
        ...
        if (null != this.activities[i][j]) {
          for (var k = j + 1; j < this.activities[i].length; k++) {
            if (null != this.activities[i][k]) {
              if (this.activities[i][j].subject == this.activities[i][k].subject) {
                this.activities[i][j].vaildWeeks = bitor(this.activities[i][j].vaildWeeks,this.activities[i][k].vaildWeeks);
 ...
 * (user,subject,places). then output marshal vaildWeek string . if course is
 * null. the course subject will be omitted in last string. style is
 * -------------------------------- | user1 subject1 | |
 * (odd1-2,places1) | | (even2-4,places2) | | user2 subject1 | |
 * (odd3-6,places1) | | (even5-8,places2) |
 * ----------------------------------
 *
 * @param index
 *      time unit index
 */
function marshalActivity(index) {
  if (this.activities[index].length === 0) {
    return "";
  }
  if (this.activities[index].length == 1) {
    var cname = this.activities[index][0].subject;
    var tname = this.activities[index][0].users;
    ...
    // merge this.activities to tempActivities by same subject and places .start with 1.
    for (var i = 1; i < this.activities[index].length; i++) {
      ...
      for (var j = 0; j < tempActivities.length; j++) {
        if (this.activities[index][i].canMergeWith(tempActivities[j])) {
          ...
          tempActivities[j].vaildWeeks = bitor(tempActivities[j].vaildWeeks, secondWeeks);
        }
      }
      if (!merged) {
    ...
    for (var m = 0; m < tempActivities.length; m++) {
      ...
      var subject = tempActivities[m].subject;
      var users = tempActivities[m].users;
      // add users and subject
      if (subject !== null) {
        marshalString += delimiter + subject ;
      }
      marshalString += delimiter + users + "(" + tempActivities[m].marshal()  + ")";
      for (var n = m + 1; n < tempActivities.length; n++) {
        // marshal same subject activity
        if (tempActivities[n] !== null && subject == tempActivities[n].subject) {
          marshalString += delimiter + tempActivities[n].users + "(" + tempActivities[n].marshal() + ")";
...
// new activity
function ActivityInfo(subject, users, places, vaildWeeks, comments) {
...
function cloneActivity() {
  return new ActivityInfo(this.subject, this.users, this.places, this.vaildWeeks, this.comments);
}
//
...
/*******************************************************************************
 * course table display occupy of users,places. It also
 * represent data model of any course arrangement. For example student's course
 * table,single course's table,users course table,and adminClass's course
 * table,even major's .
 ******************************************************************************/
function CourseTable(beginOn, courseUnits) {
  ...
  this.newActivity = function(subject, users, places, startOn, weekstate, comments) {
    var weekstate_str = "";
    if (startOn) {
      var weeks = Dates.weeksBetween(this.beginOn.getDay(), this.beginOn, Dates.toDate(startOn));
      weekstate_str = this.convertWeekstate2ReverseString(weekstate,  weeks);
    }
    return new ActivityInfo(subject, users, places, weekstate_str, comments)
  }
  ...
  this.detectCollisionInCell = function(activitiesInCell) {
    ...
    var cellSubjects = new Object();
    var mergedValidWeeks = activitiesInCell[0].vaildWeeks.split("");
    // 登记第一个课程
    cellSubjects[activitiesInCell[0].subject] = true;
    for (var z = 1; z < activitiesInCell.length; z++) {
      ...
      for (var x = 0; x < mergedValidWeeks.length; x++) { // 53代表53周
        ...
        if (m + t <= 1) {
          ...
          // 如果已经是登记过的，则不算冲突
          if (!cellSubjects[activitiesInCell[z].subject]) {
            return true; // 发现冲突
          }
        }
      }
      // 登记该课程
      cellSubjects[activitiesInCell[z].subject] = true;
    }
    return false;
  }
