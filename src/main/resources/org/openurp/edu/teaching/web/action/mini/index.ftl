...
<div class="container-fluid">
  ...
  <div class="card card-primary card-outline">
    <div class="card-header">
      <h3 class="card-title">[@ems.avatar username=teacher.code/]${teacher.name}的主课课程安排</h3>
    ...
    <div class="card-body" style="padding-top:0px;">
      ...
      <table class="grid-table" id="occupy-table" style="font-size: 12px;">
        <caption style="caption-side: top;text-align: center;padding-top: 0px;padding-bottom: 0px;">导师课表(课程+主课)</caption>
  ...
  <div class="card">
    <div class="card-body">
      <div style="display:flex">
        <div style="margin:0px 20px 0px 10px;">[@ems.avatar username=std.code style="border-radius: 10%;width:50px;"/]</div>
        <div>
          <h6>${std.name} ${std.grade.name}级 ${std.level.name}</h6>
          <p style="margin:0px">
            <span class="text-muted">学号：</span>${std.code}
          ...
          [#if term>0]
            ...
            [#if clazzMap.get(course)?? && clazzMap.get(course).get(std)??]
              [#assign clz = clazzMap.get(course).get(std)/]
                [#assign scheduleHours=clz.scheduleHours/]
                <span class="text-muted">课时：</span><span [#if scheduleHours != 17*2]class="text-danger" title="课时应为34"[/#if]>${scheduleHours}课时</span>
                <br/>
              [#assign units=[]/]
              [#list clz.activities as act]
                [#assign unit]${act.time.weekday.id}_${act.beginUnit}_${act.endUnit}[/#assign]
                [#if units?seq_contains(unit)][#continue/][/#if]
                ...
                <div>
                  ...
                  [@b.a href="!remove?clazz.id=${clz.id}&unit=${unit}"
                        class="btn btn-sm btn-link" onclick="if(confirm('确定删除该课程的本次安排吗?')){return bg.Go(this,null)}else{return false;}"]<i class="fa-solid fa-trash-can"></i>删除[/@]
                  [#if !act_has_next && clz.scheduleHours<17*2]
